...
const flightSearchURL =
  'https://apidojo-hipmunk-v1.p.rapidapi.com/flights/create-session';
...
function getFlightResults() {
  ...
  const departureLocation0 = $('.airport0')
    .val()
    .toUpperCase();
  const departureLocation1 = $('.airport1')
    .val()
    .toUpperCase();
  ...
  fetch(urlFlight1, {
    headers: {
      'X-RapidAPI-Key': '37815a5062mshb00a5a1e7f13f85p1e7925jsn183b12ff34e5'
    }
  })
    .then(response => {
      if (response.status == 503) {
        $('.ifError').append(
          `<p class='errorMessage'>Sorry, something went wrong! Please try your search again.</p>`
        );
        throw new Error(response.statusText);
      } else if (flightCodeList.indexOf(destination) == -1) {
        $('.ifError').append(
          `<p class='errorMessage'>Please enter a valid US destination airport code.</p>`
        );
        throw new Error(response.statusText);
      } else {
  ...
  fetch(urlFlight2, {
    ...
    .then(response => {
      if (flightCodeList.indexOf(departureLocation1) == -1) {
        $('.ifError').append(
          `<p class='errorMessage'>Please enter a valid US airport code for traveler 2.</p>`
        );
        throw new Error(response.statusText);
      } else {
        return response.json();
  ...
  fetch(urlFlight3, {
    ...
    .then(response => {
      if (flightCodeList.indexOf(departureLocation0) == -1) {
        $('.ifError').append(
          `<p class='errorMessage'>Please enter a valid US airport code for traveler 1.</p>`
        );
        throw new Error(response.statusText);
      } else {
        return response.json();
