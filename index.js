...
function getFlightResults(departureClass, destinationClass) {
    const departureLocation = $(departureClass).val().toUpperCase();
    const destination = $(destinationClass).val().toUpperCase();
    const departureDate = departureDateFormat();

    const paramsFlight = {
            from0: departureLocation,
            to0: destination,
            date0: departureDate,
            pax: 1,
            cabin: 'coach'
            }
    const queryString = formatQueryParams(paramsFlight);
    const urlFlight = flightSearchURL + '?' + queryString;
    fetch(urlFlight, {
        ...
        .catch(error => console.log('Flight: API not working: ' + error));    
}
